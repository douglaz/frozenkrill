...
pub fn complete_words_with_checksum(
    ...
    for word in words {
        if let Ok(idx) = WORDS.binary_search(&word.as_str()) {
            mnemonic.push(idx);
    ...
    if words.len() == 11 {
        writer.write(rng.next_u64(), 7)?;
    } else {
        // 23 words
        writer.write(rng.next_u64(), 3)?;
    }
    writer.flush()?;
    ...
    for _ in 0..mlen {
        let i: usize = reader.read(11)?.try_into()?;
        mnemonic.push(WORDS[i].to_owned());
    }
    Ok(mnemonic)
}
...
mod tests {
    ...
    #[test]
    fn test_complete_words_with_checksum() -> anyhow::Result<()> {
        let mut rng = rand::thread_rng();
        for word_count in [24, 12] {
            let seed_phrase =
                Mnemonic::generate_in(bip39::Language::English, word_count)?.to_string();
            println!("original seed phrase: {seed_phrase}");
            let seed_phrase: Vec<_> = seed_phrase.split(' ').map(ToString::to_string).collect();
            let last = seed_phrase.len();
            let without_last_word = &seed_phrase[0..last - 1];
            let with_checksum = complete_words_with_checksum(&mut rng, without_last_word)?;
            let new_seed_phrase = with_checksum.join(" ");
            println!("modified seed phrase: {new_seed_phrase}");
            Mnemonic::from_str(&new_seed_phrase)?;
        }
        Ok(())
    }
}
